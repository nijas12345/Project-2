...
import {
  resendAdminResetPassword,
  resetAdminPassword,
} from "../services/adminApi/adminAuthService";

export const AdminForgotPage: React.FC = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        setMessage(error.message); // This will be your server-sent message
      } else {
  ...
  const handleResend = async () => {
    setIsSubmitting(true);
    try {
      await resendAdminResetPassword(email);
